...
def main(lochEdge, n, startingLng, startingLat, loch):
    # Task 1 + Make GeoJson polygon from list of pts.
    startingLoc = (float(startingLng), float(startingLat))
    ...
    X = np.array(data)
    kmeans = KMeans(n_clusters = int(n), random_state = 0, max_iter = 1000).fit(X)
    ...
    file_to_write = 'firstJourney' + loch + '.geojson'
    ...
    random_df.to_csv(loch + '_rand.csv')
    centroids_df.to_csv(loch + '_centroids.csv')
    startingLoc_df.to_csv(loch + '_startingLoc.csv')
